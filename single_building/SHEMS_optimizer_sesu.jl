function SHEMS_optimizer_sesu(sh, hp, fh, hw, b, m, bc_violations)
    ...
    df = CSV.read("data/$(m.season)_$(m.run)_$(m.price).csv", DataFrame, header=true);
    ...
    # Objective function: maximize self-sufficiency _____________________________________________________________________
    @objective(model, Min, sum(X[h,3] + X[h,7] +
    ...
    @constraint(model, SeSu_comfort, sum(T_fh_plus[h] + T_fh_minus[h] + V_hw_plus[h] + V_hw_minus[h] for h in 1:m.h_predict) <= bc_violations);
